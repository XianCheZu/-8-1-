...
@interface PayView ()<MBProgressHUDDelegate,WB_StopWatchDelegate,UIAlertViewDelegate>{
...
- (void)createMainView{
    ...
    stopLabel.frame = CGRectMake(width/2-width*0.3, CGRectGetMaxY(payMoney.frame)-width*0.05, width*0.6, width*0.2);
    stopLabel.delegate = self;
    stopLabel.font = [UIFont fontWithName:@"Arial-BoldMT" size:30];
    stopLabel.textColor = Color(255, 97, 57);
    stopLabel.textAlignment = NSTextAlignmentCenter;
    [stopLabel setTimeFormat:@"mm:ss"];
    // ************************ 时间是后台获取的 ***********************************
    [stopLabel setCountDownTime:3600];
    [stopLabel start];
...
#pragma mark - TimeOver
-(void)timerLabel:(WB_Stopwatch *)timerLabel finshedCountDownTimerWithTime:(NSTimeInterval)countTime{
    
    NSLog(@"计时结束");
    NSDictionary *dic = @{@"userid":[ZCUserData share].userId,@"id":_Dingdan};
    [HttpManager postData:dic andUrl:@"http://wx.leisurecarlease.com/api.php?op=api_delorder" success:^(NSDictionary *fanhuicanshu) {
