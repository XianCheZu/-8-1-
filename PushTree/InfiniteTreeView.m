...
- (void)awakeFromNib
{
    ...
    _lineView = [[UIView alloc] initWithFrame:CGRectMake(Height(100), 0, 1, CGRectGetHeight(self.bounds))];
...
- (void)applyNextAnimation:(InfiniteTreeTableView*)tableView currentLevel:(NSInteger)currentLevel
{
    ...
    frame.origin.x = Height(100);
    frame.size.width = Height(220);
...
- (void)applyBackAnimation:(InfiniteTreeTableView*)tableView currentLevel:(NSInteger)currentLevel
{
    ...
    if (currentLevel >= 2) {
        ...
        secondFrame.origin.x = Height(100);
        secondFrame.size.width = Height(220);
        [UIView animateWithDuration:0.3 animations:^{
            _lineView.left = self.frame.size.width;
            lastTableView.left = self.frame.size.width;
    ...
    if (!_animating) {
        ...
        if ([_delegate respondsToSelector:@selector(pushTreeViewWillReloadAtLevel:currentLevel:level:indexPath:)]) {
            ...
            if (needReloadDataLevel <= 0) {
                ...
                tableView.frame = CGRectMake(0, 0,Height(220), CGRectGetHeight(self.bounds));
            }
