...
//    self.navigationItem.title = @"城市选择";
    ...
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width*0.4, 40)];
    title.text = @"城市选择";
...
//    UIBarButtonItem *cancelBarButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"返回11.png"] style:UIBarButtonItemStylePlain target:self action:@selector(cancelButtonDown:)];
    UIButton *fanhui = [UIButton buttonWithType:UIButtonTypeCustom];
    fanhui.frame = CGRectMake(0, 0, 25, 25);
    [fanhui addTarget:self action:@selector(cancelButtonDown:) forControlEvents:UIControlEventTouchUpInside];
...
- (NSMutableArray *) localCityData
{
    if (_localCityData == nil) {
        ...
        NSLog(@"self.locationCityID=%@",self.locationCityName);
        if (self.locationCityName != nil) {
            TLCity *city = nil;
            //            NSLog(@"self.cityData=%@",self.cityData);
