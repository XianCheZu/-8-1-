...
-(void)refresh
{
    ...
    self.titleLabel.tintColor = _isChoose ? _chooseColor : _normalColor;
}
...
-(NSString*)chooseString
{
    ...
    for (ChooseItem * label in _itemArray)
    {
        if (label.isChoose)
        {
            [chooseTextArray addObject:label.titleLabel.text];
        }
    }
...
//        CGFloat labelWidth = [TPLHelpTool lengthOfString:[_titleArray objectAtIndex:i] withFont:titleFont];
        ...
        if (labelWidth <= tempWidth)//能放下
        {
            ...
            [label setTintColor:_itemTextColor] ;
            label.tag=i;
            [label addTarget:self action:@selector(anniu:) forControlEvents:UIControlEventTouchUpInside];
            [label setTitle:[_titleArray objectAtIndex:i] forState:UIControlStateNormal];
            label.titleLabel.font = _itemFont;
...
-(void)anniu:(UIButton *)sender
{
    NSLog(@"4444=%@",(UIButton *)[sender currentTitle]);
    NSString *QIN=[NSString stringWithFormat:@"%@",(UIButton *)[sender currentTitle]];
    if (_bianqian) {
        _bianqian(QIN);
    }
}
...
//    NSString * systemString = [TPLHelpTool getSystemVersion];
    ...
    {
        size = [string sizeWithAttributes:font];
    }
