...
-(UITableView *)tableView  {
    if (_tableView == nil) {
        _tableView=[[UITableView alloc]initWithFrame:CGRectMake(0, screen_width*0.7+64+screen_width*0.13, screen_width, screen_height-screen_width*(0.13+0.7)-64) style:UITableViewStylePlain];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self sendAddressString:[NSString stringWithFormat:@"%@",poi.name]];
...
-(void)sendAddressString:(NSString *)string {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(postAddressString:)]) {
        
        [_delegate postAddressString:string];
