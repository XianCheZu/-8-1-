...
-(void)tiJiaoButton:(UIButton *)tiJiao {
    ...
    NSDictionary *postdict= @{@"userid":[ZCUserData share].userId,@"username":[ZCUserData share].username,@"0":image1Base64,@"1":image2Base64,@"2":image3Base64,@"text":_textView.text};
    // 开始上传
    ...
    AFHTTPRequestOperation *operation = [manager POST:@"http://wx.leisurecarlease.com/api.php?op=api_feedback" parameters:postdict constructingBodyWithBlock:^(id<AFMultipartFormData> formData){
        ...
        [formData appendPartWithFileData:imageData2 name:@"1" fileName:@"pic2.jpg" mimeType:@"image/jpeg/png/jpg"];
        [formData appendPartWithFileData:imageData3 name:@"2" fileName:@"pic3.jpg" mimeType:@"image/jpeg/png/jpg"];
...
-(void)jieTuButtonClick:(UIButton *)jieTuBtn {
    ...
    if (flag<3) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"最多只能传三张图片" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alert show];
    }
}
